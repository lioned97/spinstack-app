...
const DEFAULT_GEMINI_KEY = "AQ.Ab8RN6JNHw48arZpS5ph52aGyNk2JArXpjUxUFx2N9rfXRb2Ig";
...
export default function SpinStack() {
  ...
  const scoreCard = (paper) => {
    ...
    // 2. Dynamic Affinity Accumulators from User History
    if (profile.topics) {
    ...
    // 3. Temporal Balance Framework (2026 Active Target Baseline)
    const currentYear = 2026;
    const publicationYear = parseInt(paper.year) || currentYear;
    if (publicationYear >= currentYear) score *= 1.25;
    else if (publicationYear === currentYear - 1) score *= 1.1;
    else if (publicationYear < currentYear - 3) score *= 0.65; // Deprecate legacy data paradigms
  ...
  const handleSwipe = (isKept) => {
    ...
    // 1. Commit Preferences to Swipe Learning Context Profile
    let profile = sGet('ss_affinity_profile') || { topics: {}, authors: {} };
    const dynamicDelta = isKept ? 0.35 : -0.2;

    const coreKeywords = ['nv-center', 'decoherence', 'quantum sensing', 'open-quantum', 'optics'];
    const paperContextText = `${currentPaper.title} ${currentPaper.abstract}`.toLowerCase();

    coreKeywords.forEach(kw => {
      if (paperContextText.includes(kw.replace('-', ' '))) {
        profile.topics[kw] = (profile.topics[kw] || 0) + dynamicDelta;
      }
    });
    ...
    // 2. Add to Saved Collection if Accepted
    if (isKept) {
    ...
    // 4. Synchronize state with Supabase storage
    pushToCloud();
  };
  ...
  const triggerFeedHarvest = async () => {
    setSyncLoading(true);
    try {
      const response = await fetch(HARVEST_URL);
      if (!response.ok) throw new Error("Harvest stream connection failed.");
      const data = await response.json();
      
      const scoredAndSorted = sortStackByAffinity(data);
      setStack(scoredAndSorted);
      persist('ss_papers_pool', scoredAndSorted);
      setCurrentIndex(0);
    } catch (err) {
  ...
  const handleAddTopic = async (e) => {
    ...
    // CORS-safe discovery query matching targets instantly
    ...
    try {
      ...
      const injectedEntries = data.results.map(record => ({
        id: record.id,
        title: record.display_name,
        abstract: record.abstract_inverted_index 
          ? `Abstract matrix reconstruction placeholder for modern synthesis [${record.publication_year}].` 
          : "Abstract metadata string format missing or protected.",
        authors: record.authorships?.map(a => a.author.display_name) || ["Unassigned Research Team"],
        year: record.publication_year,
        venue: record.primary_location?.source?.display_name || "OpenAlex Engine",
        url: record.doi || record.id
      }));

      const combinedPool = sortStackByAffinity([...injectedEntries, ...stack]);
      setStack(combinedPool);
      persist('ss_papers_pool', combinedPool);
      setCurrentIndex(0); // Reset viewing layout matrix back to head of card view
      pushToCloud();
    } catch (e) {
  ...
  // --- Secure Built-In Gemini Processing Engine ---
  const triggerGeminiAnalysis = async () => {
    ...
    const settings = sGet('ss_settings') || {};
    const apiKey = settings.apiKey || DEFAULT_GEMINI_KEY;

    // Production stable model mapping path string for 2026 operations
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 selection:bg-cyan-500 selection:text-slate-950 flex flex-col font-sans antialiased">
      {isDesktop ? (
        /* =========================================================================
